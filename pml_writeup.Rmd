...
train_model <- function(algo_name, train_data){
  tC <- trainControl(method = "repeatedcv", repeats = 2)
  train(classe ~ ., data = train_data, method = algo_name, trainControl = tC)
}
library(doParallel)
...
I ended up choosing the Random Forest classifier. It eventually ended up producing a perfect score on `pml-testing.csv`.  Because the Expected Out-of-Sample error was so low (0.3%), I didn't need to re-evaluate my approach.  However, if I did, I would have split the `training-set` further to produe a `validation-set` to tune my approach toward.  Ostensibly, the cross validation I specify in `trainControl` should be performing cross validation.  However, when the model is examined after calling `train`, "Bootstrapped" is shown as the resampling method.  More information on this is provided in the "Caret Untrustworthiness" section below.      
